...
build_spirv_cross() {
    ...
    cmake .. \
        ...
        -DSPIRV_CROSS_FORCE_ENABLE_EXCEPTIONS=OFF

    cmake --build . --config Release -j$(nproc)

    # Install manually (CMake install sometimes tries to run host tools; keep it simple)
    mkdir -p "$INSTALL_PREFIX/spirv-cross/lib" "$INSTALL_PREFIX/spirv-cross/include"

    # Libraries can have a few names depending on version; copy what exists
    find . -maxdepth 1 -type f -name "*.lib" -print -exec cp -f {} "$INSTALL_PREFIX/spirv-cross/lib/" \\;

    # Headers
    cp -r ../include/* "$INSTALL_PREFIX/spirv-cross/include/" 2>/dev/null || true
...
build_glslang() {
    ...
    # Ensure submodules for glslang itself
    ...
    # Install manually: copy libs + public headers
    mkdir -p "$INSTALL_PREFIX/glslang/lib" "$INSTALL_PREFIX/glslang/include"

    find . -type f -name "*.lib" -print -exec cp -f {} "$INSTALL_PREFIX/glslang/lib/" \\;

    # Public headers used by consumers
    if [[ -d ../glslang ]]; then
        mkdir -p "$INSTALL_PREFIX/glslang/include/glslang"
        cp -r ../glslang/* "$INSTALL_PREFIX/glslang/include/glslang/" 2>/dev/null || true
    fi
    if [[ -d ../SPIRV ]]; then
        mkdir -p "$INSTALL_PREFIX/glslang/include/SPIRV"
        cp -r ../SPIRV/* "$INSTALL_PREFIX/glslang/include/SPIRV/" 2>/dev/null || true
    fi
...
case "$1" in
    ...
    "all")
        ...
        ;;
    *)
        echo "Usage: $0 {setup|test|zlib|libpng|spirv-cross|glslang|adlmidi|all}"
        echo ""
        ...
        echo "  spirv-cross - Build SPIRV-Cross only"
        echo "  glslang     - Build glslang only"

...
#include "fastlink/memserve/memserve.h"
#include "fastlink/memshare/memshare.h"
						 ...
						 LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
        ...
        case ID_SOUND_SYNC:
			...
			S9xDisplayStateChange (WINPROC_SYNC_SND, false);
            break;
...
static bool LoadROM(const TCHAR *filename, const TCHAR *filename2 /*= NULL*/) {
	...
	if (!Settings.StopEmulation) {
		// Entrypoint for new socket server feature
		//..
...
BOOL CreateToolTip(int toolID, HWND hDlg, const TCHAR* pText)
{
   ...
   if (!hwndTool || !hwndTip)
   {
	...
	toolInfo.lpszText = const_cast<LPTSTR>(pText);		//toolInfo.lpszText = pText;
...
INT_PTR CALLBACK DlgSoundConf(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(msg)
	{
	    case WM_INITDIALOG:
        {
			...
			SendDlgItemMessage(hDlg, IDC_SYNC_TO_SOUND_CPU, BM_SETCHECK, BST_UNCHECKED, 0);
...
INT_PTR CALLBACK DlgOpenROMProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		WinRefreshDisplay();
		{
			...
			wcsncpy(tempBuffer, ROM_COLUMN_FILENAME, _countof(tempBuffer) - 1);
			tempBuffer[_countof(tempBuffer) - 1] = L'\0';		//static const LPTSTR temp1 = ROM_COLUMN_FILENAME;
			memset(&col, 0, sizeof(LVCOLUMN));
			...
			col.cchTextMax=5;
			col.pszText = tempBuffer;
			std::fill_n(tempBuffer, 256, L'\0');		//col.pszText=temp1;

			ListView_InsertColumn(romList,    0,   &col);

			wcsncpy(tempBuffer, ROM_COLUMN_DESCRIPTION, _countof(tempBuffer) - 1);
			tempBuffer[_countof(tempBuffer) - 1] = L'\0';		//static const LPTSTR temp2 = ROM_COLUMN_DESCRIPTION;
			memset(&col, 0, sizeof(LVCOLUMN));
			...
			col.cchTextMax=32;
			col.pszText = tempBuffer;
			std::fill_n(tempBuffer, 256, L'\0');		//col.pszText=temp2;
			col.iSubItem=1;

			ListView_InsertColumn(romList,    1,   &col);


			wcsncpy(tempBuffer, ROM_COLUMN_SIZE, _countof(tempBuffer) - 1);
			tempBuffer[_countof(tempBuffer) - 1] = L'\0';		//static const LPTSTR temp3 = ROM_COLUMN_SIZE;
			memset(&col, 0, sizeof(LVCOLUMN));
			col.mask=LVCF_FMT|LVCF_ORDER|LVCF_TEXT|LVCF_WIDTH|LVCF_SUBITEM;
			col.fmt=LVCFMT_LEFT;
			col.iOrder=2;
			...
			col.cchTextMax=32;
			col.pszText = tempBuffer;
			std::fill_n(tempBuffer, 256, L'\0');		//col.pszText=temp3;
			col.iSubItem=2;
...
bool RegisterExt(TCHAR *ext) {
	...
	if (!ext || *ext == TEXT('\0'))	//if(!ext || ext==TEXT('\0'))
        return false;
...
INT_PTR CALLBACK DlgFunky(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
    {
		...
		InsertOutputMethod((LPARAM)TEXT("Direct3D11"), DIRECT3D);

...
#include "core/util/port.h"
#include "core/util/snes9x.h"
#include "wsnes9x.h"
#include "wlanguage.h"
#include "core/video-common/display.h"
#include "core/util/conffile.h"
#include "core/cartridge/spc7110.h"
...
{
	if (!conf.Exists("Display\\Win::Direct3D11:Shader") && conf.Exists("Display\\Win::Direct3D:D3DShader"))
	{
		conf.SetString("Display\\Win::Direct3D11:Shader", conf.GetString("Display\\Win::Direct3D:D3DShader"), "migrated from legacy Direct3D key");
	}
	conf.DeleteKey("Display\\Win::Direct3D:D3DShader");
...
#define CATEGORY "Display\\Win"
	AddUIntC("OutputMethod", GUI.outputMethod, 0, "0=Direct3D11, 2=Vulkan");
	...
	AddStringC("Direct3D11:Shader", GUI.D3DshaderFileName, MAX_PATH, "", "shader filename for Direct3D11 mode (HLSL file)");
	AddStringC("Vulkan:Shader", GUI.VulkanShaderFileName, MAX_PATH, "", "shader filename for Vulkan mode (.glslp or .slangp)");
...
#define	CATEGORY "Sound"
	AddIntC("Sync", Settings.SoundSync, 0, "legacy setting; forced off in FastLink non-blocking pipeline.");
	...
	AddBoolC("DynamicRateControl", Settings.DynamicRateControl, true, "enabled with bounded low limit in FastLink Win32 to prevent long-run drift clicks.");
...
#define	CATEGORY "Sound\\Win"
	AddUIntC("SoundDriver", GUI.SoundDriver, 9, "9=WASAPI (low latency, only supported backend)");
	AddUIntC("BufferSize", GUI.SoundBufferSize, 16, "sound buffer size in ms - determines the internal and output sound buffer sizes. actual mixing is done every SoundBufferSize/4 samples");
	...
	AddStringC("OutputDevice", GUI.AudioDevice, MAX_AUDIO_NAME_LENGTH, "Default", "Name of the output audio device (WASAPI). Set to 'Default' for the system default audio device");
#undef CATEGORY
...
#undef CATEGORY
#define	CATEGORY "MemoryMapServe"
	AddBool("Enabled", Settings.MemoryMapServe, true);
#undef CATEGORY
}

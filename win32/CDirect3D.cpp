...
void CDirect3D::CreateDrawSurface()
{
	...
	if(!drawSurface) {
		...
		if(FAILED(hr)) {
			...
			char dbgbuf[256]; snprintf(dbgbuf, sizeof(dbgbuf), "[SNES9X-D3D] Error while creating texture hr=0x%08X", hr); OutputDebugStringA(dbgbuf); MessageBox(nullptr, TEXT("Error while creating texture"), TEXT("Error"), MB_ICONERROR | MB_OK);
			return;
		}
	}
}

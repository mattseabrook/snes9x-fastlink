...
bool CVulkan::Initialize(HWND hWnd)
{
    ...
    try {
        ...
        if (!context->init_win32(0, hWnd))
            { VDbg("Return false at line %d", __LINE__); return false; }
    }
    catch (std::exception& e)
    {
        VDbg("Exception in CVulkan::Initialize: %s", e.what());
        VDbg("Return false at line %d", __LINE__); return false;
    }
...
void CVulkan::Render(SSurface Src)
{
    ...
    if (!result && simple_output)
    {
        ...
        if (!result) { VDbg("simple_output->do_frame_without_swap failed"); }
    }
    ...
    {
        VDbg("Render result was false, skipping swap");
    }
}
